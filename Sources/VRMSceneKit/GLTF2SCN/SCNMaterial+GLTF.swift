...
//        lightingModel = .physicallyBased
        ...
        if let pbr = material.pbrMetallicRoughness {
            ...
            if let metallicTexture = pbr.metallicRoughnessTexture {
                ...
                let image = try metalness.contents as? GLTImage ??? ._dataInconsistent("failed to load texture image")
    ...
    private func createMetallicRoughnessTexture(from uiImage: GLTImage) throws -> (metal: GLTImage, rough: GLTImage) {
    ...
    private func createGraySpaceImage(width: Int,
                                      height: Int,
                                      dataPointer: UnsafeMutablePointer<UInt8>) throws -> GLTImage {
